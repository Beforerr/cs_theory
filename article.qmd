---
title: On a Alfvenicity of solar wind discontinuities
authors:
  - name: Zijin Zhang
...
Three examples of solar wind discontinuities observed by Parker Solar Probe (PSP), ARTEMIS and Wind spacecraft with sub-Alfvenic flow velocity.
:::
...
The governing equations for the multi-fluid plasma model can be expressed in conservation law form as follows:
...
$$
m_α n_α (\frac{∂}{∂ t} + \mathbf{u}_{α} · ∇) \mathbf{u}_α  = - ∇ · \mathbf{P}_α + q_α n_α \left( \mathbf{E} + \frac{\mathbf{u}_α \times \mathbf{B}}{c} \right) + \mathbf{R}_α
$$

where $α$ indicates the particle species, $n_α$ the number density, $m_α$ the mass, $q_α$ the charge, $\mathbf{u}_α$ the bulk velocity, $\mathbf{P}_α$ the pressure tensor, $\mathbf{E}$ the electric field, $\mathbf{B}$ the magnetic induction (magnetic field), and $R_α$ the friction forces.

In this study, we focus on the equilibrium solutions of the set of equations, which implies that the partial derivatives with respect to time are set to zero ($\frac{∂}{∂ t}=0$). 
Our primary interest lies in the current sheet, and we simplify the problem by assuming that the magnetic field depends only on a single Cartesian coordinate, designated as $z$.
Additionally, we assume that the background electron is massless, meaning the electrons follow the magnetic field lines $\mathbf{u}_e \times \mathbf{B} = 0$. 
For further simplification, we also assume the ion populations are gyrotropic, eliminating any non-diagonal terms in the pressure tensor.
...
$$
d B_z / d z = J_z = \sum_α n_α u_{α z} - n_e u_{e z} = n u_z - Γ_e
$$ {#eq-Jz}
